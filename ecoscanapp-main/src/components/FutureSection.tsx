        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-6 max-w-6xl mx-auto">
          {plans.map((p, i) => (
            <FadeInSection key={i} delay={i * 100}>
              <div className="bg-card rounded-xl sm:rounded-2xl p-5 sm:p-6 md:p-7 border border-border text-center hover:-translate-y-1 transition-transform duration-300 hover:shadow-lg h-full flex flex-col">
                <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-xl sm:rounded-2xl bg-primary/10 flex items-center justify-center mx-auto mb-3 sm:mb-4 md:mb-5 flex-shrink-0">
                  <p.icon className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-primary" />
                </div>
                <h3 className="font-bold text-base sm:text-lg md:text-xl mb-2 sm:mb-3 text-foreground">
                  {p.title}
                </h3>
